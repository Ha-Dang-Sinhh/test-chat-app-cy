    ...
    <div class="w-4/6 mx-auto border mt-20 rounded lg:grid lg:grid-cols-3">
      <div class="lg:col-span-3 lg:block">
        <div class="w-full">
            ...
            <ul class="space-y-2">
              <li :class="user.uid == mes.uid ? 'flex justify-end':'flex justify-start'" v-for="(mes,index) in dataMessage.sort((a,b)=>a.date - b.date)" :key="index">
                <div>
                  ...
                  <div v-if="mes.urls.length > 0" :class="user.uid == mes.uid ? 'mt-1 flex justify-end' : 'mt-1 flex justify-start'">
          ...
          <form @submit.prevent="" class="flex items-center justify-between w-full p-3 border-t border-gray-300 ">
